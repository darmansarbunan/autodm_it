...
class TwitterBot:
            ...
            if dmsender == self.me.id:
                ...
                while len(tweet) > 280:
                    ...
                    if complete == 0:
                        ...
                        complete = api.update_status(tweet1, in_reply_to_status_id=complete,
    ...
    def senddm(self, i, dmsender, status, postid=None, rttime=None):
        ...
        if status == 'sent':
            message = {'sent': 'Post was successfully sent at ' + rttime.astimezone(
                timezone(timedelta(hours=config.timezone))).strftime(
                "%Y-%m-%d %H:%M") + ' WIB. Check your post here: ' + url}
        elif status == 'notsent':
            message = {'notsent': 'Post was not sent. Use the trigger ' + config.trigger + ' to send post.'}
        else:
            message = {'wrong attachment': 'Post was not sent. Send only picture attachment (not gif/video).'}
